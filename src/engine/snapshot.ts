...
function detectVisiblePageText(store: AgentStore, maxLen = 4000): string {
  ...
  if (!topEl) return "";
  const raw = (topEl.innerText ?? topEl.textContent ?? "")
    .replace(/\s+\n/g, "\n")
    .trim();
            ...
            Object.fromEntries(
              Object.entries(snap.injections).map(([targetId, list]) => [
                ...
                list.map((i) => ({
                  ...
                  position: i.position,
                  htmlPreview:
                    i.html.length > 80 ? i.html.slice(0, 80) + "…" : i.html,
                })),
              ]),
            ),
            null,
